...
#
# refresh_grammars.sh calls this script on its own, for every version that has no column
# definitions yet, and it starts a server of that version first. Run that one instead unless
# you need a particular test suite or a particular server.
#
...
# fails. Writes <version>_coldefs.txt into the yacc directory, beside the grammar files.

if [ -z "${2}" ]; then
  echo "Usage: ${0} <mysql-test directory> <version> [socket]"
  echo "refresh_grammars.sh calls this script for every version that has no column definitions yet, and starts a server of that version first. Run that one instead unless you need a particular test suite or a particular server"
  exit 1
fi
TESTDIR="${1}"
VERSION="${2}"
SOCKET="${3}"
OUTDIR="$(cd "$(dirname "${0}")/../yacc" 2>/dev/null && pwd)"
if [ -z "${OUTDIR}" ]; then
  echo "Assert: the yacc directory was not found next to $(dirname "${0}"). Run this from a mariadb-qa checkout"
  exit 1
fi
