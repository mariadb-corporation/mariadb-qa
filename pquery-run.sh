...
pre_shuffle_setup(){
  ...
  if [ "${PRE_SHUFFLE_SQL}" == "1" ]; then
    shuf --random-source=/dev/urandom -n ${PRE_SHUFFLE_MIN_SQL_LINES} ${INFILE} | grep --binary-files=text -hivE "${ADV_FILTER_LIST}" | sed 's|/data/[^:]\+\.sql:||g;s|/test/[^:]\+\.sql:||g;s|;#NOERROR$|;|;s;|;#NOERROR#.*$|;|;s|;#ERROR: .*$|;|' > ${INFILE_SHUFFLED}  # Final sed: remove any erronous /data or /test prefixes and previous pquery outcomes (also another one below v)
    ...
    sed -i 's|/data/[^:]\+\.sql:||g;s|/test/[^:]\+\.sql:||g;s|;#NOERROR$|;|;s;|;#NOERROR#.*$|;|;s|;#ERROR: .*$|;|' ${INFILE_SHUFFLED}  # Remove any erronous /data or /test prefixes and previous pquery outcomes (also another one above ^)
    PRE_SHUFFLE_RES_FIN_LINES="$(wc -l ${INFILE_SHUFFLED} | awk '{print $1}')"
    if [ "${PRE_SHUFFLE_RES_FIN_LINES}" -eq 0 ]; then
