...
find_san_bug(){
  ...
  echo "${BUG_STRING}" | if grep -Fq "no core file found" ; then echo  "${VERSION} ${2}: No SAN issue detected" ; \
   elif echo ${BUG_STRING} | cut -d '|' -f1 | grep -Fq "SAN"; then echo "${VERSION} ${2}: ${BUG_STRING}" ; \
   else echo "${VERSION} ${2}: No SAN issue detected, though saw ${BUG_STRING}"; fi
  cd "${TSB_PWD}"
}
