...
grep --binary-files=text -n -iE 'RELEASE|KILL|USER|REVOKE|ROOT|SHUTDOWN|PRIVILEGES|_PRIV|DROP.*host|DROP.*user' "${1}" 

echo '====== Most likely possibilties (check lines just before these manually in "All possibilties" output above)'
grep --binary-files=text -n -iE ' RELEASE|KILL |USER.*ROOT|REVOKE.*ROOT|SHUTDOWN|DROP.*mysql.*host|DROP.*mysql.*user' "${1}" 
