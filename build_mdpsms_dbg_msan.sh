...
cd ..
rm -Rf ${CURPATH}_dbg_msan
rm -f /tmp/mdpsms_dbg_msan_build_${RANDOMD}
cp -R ${CURPATH} ${CURPATH}_dbg_msan
cd ${CURPATH}_dbg_msan
...
if [ $FB -eq 0 ]; then
  ...
  echo $CMD
  eval "$CMD" 2>&1 | tee /tmp/psms_dbg_msan_build_${RANDOMD}
...
make -j${MAKE_THREADS} 2>&1 | tee -a /tmp/psms_dbg_msan_build_${RANDOMD}
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution 2>&1 | tee -a /tmp/psms_dbg_msan_build_${RANDOMD}
