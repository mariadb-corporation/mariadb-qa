...
while read line; do
  STRING="$(echo "$line" | sed 's|[ \t]*##.*$||' |  sed 's|"|\\\\"|g')"  # For more information on the " to \" sed, ref pquery-prep-red.sh (search for:  The sed transforms "  ), and pquery-results.sh (search for:  sed reverts the insertion of  ).
