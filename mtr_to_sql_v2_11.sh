...
DIRECTORY="${1}"  # Should contain mariadb-test directory as a first level subdirectory, see TESTS_PATH
TESTS_PATH=$(echo ${DIRECTORY} | sed 's|$|/mariadb-test/|;s|//|/|g')
...
if [ ! -r ${TESTS_PATH}/t/1st.test -a ! -r ${TESTS_PATH}/main/kill.test ]; then
  if [ ! -r ${TESTS_PATH}/mariadb-test/t/1st.test -a ! -r ${TESTS_PATH}/mariadb-test/main/kill.test ]; then
    ...
    echoit "Found tests located at ${TESTS_PATH}/mariadb-test..."
    TESTS_PATH="${TESTS_PATH}/mariadb-test"
  fi
else
