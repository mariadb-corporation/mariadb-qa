...
ps -ef | egrep "maria"     | grep "$(whoami)" | egrep -v "grep|vim" | awk '{print $2}' | xargs kill -9 2>/dev/null
...
ps -ef | egrep "go-expert" | grep "$(whoami)" | egrep -v "grep|vim" | awk '{print $2}' | xargs kill -9 2>/dev/null
ps -ef | egrep "\./all"    | grep "$(whoami)" | egrep -v "grep|vim" | awk '{print $2}' | xargs kill -9 2>/dev/null
ps -ef | egrep "bug"       | grep "$(whoami)" | egrep -v "grep|vim" | awk '{print $2}' | xargs kill -9 2>/dev/null
