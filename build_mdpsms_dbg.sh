...
MAKE_THREADS=10         # Number of build threads
...
if [ $USE_AFL -eq 1 ]; then
  ...
  FLAGS='-DCMAKE_CXX_FLAGS=-march=native'  # -DCMAKE_CXX_FLAGS="-march=native" is the default for FB tre
fi
...
cd ..
rm -Rf ${CURPATH}_dbg
rm -f /tmp/mdpsms_dbg_build_${RANDOMD}
