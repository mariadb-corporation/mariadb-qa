...
# TSAN bootstrap: the install-db mariadbd --bootstrap inherits this; exitcode=0 lets it succeed despite server start-up data races, suppressions=TSAN.filter quiets them (ref MDEV-27138)
if [[ "${PWD}" == *"TSAN"* ]]; then echo "export TSAN_OPTIONS=suppressions=${SCRIPT_PWD}/TSAN.filter:suppress_equal_stacks=1:history_size=7:second_deadlock_stack=1:verbosity=1:exitcode=0" >> wipe; fi
...
# TSAN bootstrap: the install-db mariadbd --bootstrap inherits this; exitcode=0 lets it succeed despite server start-up data races, suppressions=TSAN.filter quiets them (ref MDEV-27138)
if [[ "${PWD}" == *"TSAN"* ]]; then echo "export TSAN_OPTIONS=suppressions=${SCRIPT_PWD}/TSAN.filter:suppress_equal_stacks=1:history_size=7:second_deadlock_stack=1:verbosity=1:exitcode=0" >>init; fi
...
echo "while true; do echo '------------------'; ./all_no_cl \"\${*}\" 2>&1 >/dev/null; cat ./in.sql; ./test_pquery 2>&1 >/dev/null; ${SCRIPT_PWD}/new_text_string.sh; done" >myloop && chmod +x ./myloop
...
ln -s ./all_no_cl ./anc
# Dump/binlog roundtrip checker (reducer.sh MODE=11 is the matching reduction mode)
ln -fs ${SCRIPT_PWD}/verify_dump_and_binlog.sh ./verify_dump_and_binlog
