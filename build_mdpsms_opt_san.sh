...
MAKE_THREADS=32         # Number of build threads
...
if [ $USE_SAN -eq 1 ]; then
  ...
  if [ ${USE_TSAN} -eq 1 ]; then
    ...
    if [ ${ASAN_OR_MSAN} -eq 0 ]; then
      ...
      SAN="-DWITH_UBSAN=ON"  # Spider UBSAN Only https://jira.mariadb.org/browse/MDEV-26541
      #SAN="-DWITH_ASAN=ON -DWITH_ASAN_SCOPE=ON -DWSREP_LIB_WITH_ASAN=ON"  # Spider ASAN only, same URL
    else
