...
MAKE_THREADS=15         # Number of build threads
    ...
    if [ $USE_SAN -eq 1 ]; then
      if [ $USE_CLANG -eq 1 ]; then
        ...
        if [ "$(cmake --version | grep -o '[0-9]' | head -n1)" -ge 4 ]; then  # cmake > v4.0 (also assumes Clang/LLVM 21)
          ...
          if [ ${USE_TSAN} -eq 1 ]; then
            FLAGS="-DCMAKE_C{,XX}_FLAGS='-O${O_LEVEL} -fPIC -march=native -mtune=native -fsanitize=thread -fno-omit-frame-pointer' -DCMAKE_{EXE,SHARED,MODULE}_LINKER_FLAGS='-lm -fsanitize=thread -shared-libsan -Wl,-rpath,${RTDIR}' -DCMAKE_REQUIRED_FLAGS='-fsanitize=thread -shared-libsan -Wl,-rpath,${RTDIR}'"
          else
            FLAGS="-DCMAKE_C{,XX}_FLAGS='-O${O_LEVEL} -fPIC -march=native -mtune=native -fsanitize=address,undefined -fno-omit-frame-pointer' -DCMAKE_{EXE,SHARED,MODULE}_LINKER_FLAGS='-lm -fsanitize=address,undefined -shared-libasan -Wl,-rpath,${RTDIR}' -DCMAKE_REQUIRED_FLAGS='-fsanitize=address,undefined -shared-libasan -Wl,-rpath,${RTDIR}'"
          fi
...
make -j${MAKE_THREADS} 2>&1 | tee -a /tmp/psms_opt_san_build_${RANDOMD}
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution 2>&1 | tee -a /tmp/psms_opt_san_build_${RANDOMD}
