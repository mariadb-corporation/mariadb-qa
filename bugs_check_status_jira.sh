...
grep --binary-files=text -vEi '^[ \t]*$|fixed|no parsable frames' ${HOME}/mariadb-qa/known_bugs.strings ${HOME}/mariadb-qa/known_bugs.strings.SAN | grep --binary-files=text -Eo 'MDEV-[0-9]+|MENT-[0-9]+' | sort -u | wc -l | sed 's|^|Scanning |;s|$| bugs...|'
echo "Only bugs which are fixed will be listed below, so they can be updated (add leading '# ' and '## Fixed' before MDEV number) in ${HOME}/mariadb-qa/known_bugs.strings and/or ${HOME}/mariadb-qa/known_bugs.strings.SAN :"
grep --binary-files=text -vEi '^[ \t]*$|fixed|no parsable frames' ${HOME}/mariadb-qa/known_bugs.strings ${HOME}/mariadb-qa/known_bugs.strings.SAN | grep --binary-files=text -Eo 'MDEV-[0-9]+|MENT-[0-9]+' | sort -u | xargs -I{} echo "lynx -accept_all_cookies -dump -nobold -nobrowse -nolist -nolog -nomargins -nomore -nopause -noprint -nostatus https://jira.mariadb.org/browse/{} | grep -o 'Resolution: .*' | sed 's|Resolution:[ \\t]\\+||;s|^|{}: |' | grep 'Fixed'; sleep 1" > /tmp/check_bug_status.tsh
