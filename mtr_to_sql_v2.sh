...
# Internal variables
DIRECTORY="${1}"  # Should contain mariadb-test or mysql-test directory as a first level subdirectory, see TESTS_PATH
TESTS_PATH="$(echo ${DIRECTORY} | sed 's|$|/mariadb-test/|;s|//|/|g')"
if [ ! -d "${TESTS_PATH}" ]; then
  TESTS_PATH="$(echo ${TESTS_PATH} | sed 's|mariadb-test|mysql-test|')"
fi
RANDOM=$(date +%s%N | cut -b10-19 | sed 's|^[0]\+||')  # RANDOM: Random entropy pool init
RANDOMF=$(echo $RANDOM$RANDOM$RANDOM | sed 's/..\(......\).*/\1/')
FINAL_SQL=/tmp/mtr_to_sql${RANDOMF}.sql  # Result SQL grammar (i.e. file name for output generated by this script)
