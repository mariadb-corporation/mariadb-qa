...
# RV 19-07-2023: Added '#TODO' to `grep -v` filter given the recent fixes in handling of 'other' (i.e. no NTS available failures) which are now showing. Once source is clear, update this and improve non-NTS trials handling.
${SCRIPT_PWD}/pquery-results.sh 2>/dev/null | grep --binary-files=text -vE 'TRIALS TO CHECK MANUALLY|#TODO' | sed 's|_val||g' | grep --binary-files=text -oE "Seen[ \t]+[0-9][0-9]+ times.*,.*|Seen[ \t]+[2-9] times.*,.*" | grep --binary-files=text -o "reducers [ ]*${SEARCH_STRING}" | sed "s|reducers [ ]*${SED_STRING}||" | sed 's|)||;s|,|\n|g' | grep --binary-files=text -v 'reducer' | grep --binary-files=text -v '^[ \t]*$' | sed 's|\-[1-3]*$||' | xargs -I{} ${SCRIPT_PWD}/pquery-del-trial.sh {}
