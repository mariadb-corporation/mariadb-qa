  ...
  if [ -z "${ERRORS}" -a -z "${ERRORS_LAST_LINE}" ]; then
    ...
    if [ "${2}" != "1" ]; then
      if [ "${2}" != "CHECK" ]; then
        ...
        echo "$(if [ ! -z "${ERRORS}" ]; then echo "${ERRORS}"; fi; if [ ! -z "${ERRORS_LAST_LINE}" ]; then echo "${ERRORS_LAST_LINE}"; fi;)" |  sed "s|^[-0-9: ]*||;s|[]['@/}{#\!$%\^\&\*)(]|.|g" | tr '-' '.' | sed 's|PROCEDURE [^ ]\+ |PROCEDURE.*|' | sed 's|:[0-9][0-9]\+\.|.*|' | sed 's|binlog\.0.*end_log_pos.*gtid.*Internal MariaDB error|binlog.*end_log_pos.*gtid.*Internal MariaDB error|i' | sed 's|\(gtid\) [\.0-9 ]\+|\1.*|i' # Third last sed removes port number, second last sed removes binlog file name, end_log_pos, GTID pos, last sed removes any leftover GTID pos (while leaving the upper/lower case for 'gtid' text which at times is 'GTID' and at times 'Gtid')
      fi
    else
      delete_trial
    fi
