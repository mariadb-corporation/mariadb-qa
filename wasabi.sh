...
REGEX_SQL_FILTER='debug[ \t]*=|dbug|shutdown|release|^\-\-|^let|revoke|identified|kill|root|passw|drop.*mysql|^use [^t]|master_pos_wait|delete.*user|flush privileges'  # Problematic SQL filter

# ls *.sql | xargs -I{} echo "grep --binary-files=text -viE "${REGEX_SQL_FILTER}" '{}' > '{}.new'" | tr '\n' '\0' | xargs -0 -I{} bash -c "{}" && ls *.new | xargs -I{} echo "mv {} {}" | sed 's|\.new$||' | xargs -I{} bash -c "{}"  # Do not forget 'binary-files=text' or grep will end prematurely
