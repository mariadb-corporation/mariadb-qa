...
cd ..
rm -Rf ${CURPATH}_dbg
rm -f /tmp/galera_dbg_build_${RANDOMD}
...
if [ $FB -eq 0 ]; then
  ...
  CMD="cmake . $CLANG $AFL $SSL -DCMAKE_BUILD_TYPE=Debug -DBUILD_CONFIG=mysql_release ${EXTRA_AUTO_OPTIONS} ${XPAND} -DWITH_TOKUDB=0 -DWITH_JEMALLOC=no -DFEATURE_SET=community -DDEBUG_EXTNAME=OFF -DWITH_EMBEDDED_SERVER=${WITH_EMBEDDED_SERVER} -DWITH_DEBUG_SYNC=ON -DENABLE_DOWNLOADS=1 ${BOOST} -DENABLED_LOCAL_INFILE=${WITH_LOCAL_INFILE} -DENABLE_DTRACE=0 -DWITH_SAFEMALLOC=OFF -DPLUGIN_PERFSCHEMA=${PERFSCHEMA} ${DBUG} ${ZLIB} -DWITH_ROCKSDB=${WITH_ROCKSDB} -DWITH_PAM=ON -DWITH_MARIABACKUP=1 -WITH-WSREP -DFORCE_INSOURCE_BUILD=1 ${SAN} ${FLAGS}"
...
if [[ "${TAR_dbg}" == *".tar.gz"* ]]; then
  ...
  if [ -d ${GALERA_BUILD_LOC}_dbg ]; then
    if [[ -f ${GALERA_BUILD_LOC}_dbg/libgalera_smm.so ]] || [[ -f ${GALERA_BUILD_LOC}_dbg/libgalera_enterprise_smm.so ]] ; then
      if [[ $(echo $PREFIX | cut -c1-7) == "GAL_EMD" ]]; then
        ...
        echo "ln -sf ${GALERA_BUILD_LOC}_dbg/libgalera_enterprise_smm.so ${DIR_dbg_new}/lib/libgalera_smm.so"
      else
        ln -sf ${GALERA_BUILD_LOC}_dbg/libgalera_smm.so ${DIR_dbg_new}/lib/libgalera_smm.so
        echo "ln -sf ${GALERA_BUILD_LOC}_dbg/libgalera_smm.so ${DIR_dbg_new}/lib/libgalera_smm.so"
      fi
  ...
  WSREP_COMMON_PM=${DIR_dbg_new}/mariadb-test/suite/wsrep/common.pm
