...
SCRATCH_SUFFIX='_dbg_san'
if [ ${USE_TSAN} -eq 1 ]; then SCRATCH_SUFFIX='_dbg_tsan'; fi  # Distinct scratch so TSAN and UBASAN builds of the same source can run concurrently
...
make -j${MAKE_THREADS} 2>&1 | tee -a /tmp/psms_dbg_san_build_${RANDOMD}
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution 2>&1 | tee -a /tmp/psms_dbg_san_build_${RANDOMD}
