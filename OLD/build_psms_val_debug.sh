...
cd ..
rm -Rf ${CURPATH}_val_dbg
rm -f /tmp/psms_val_debug_build_${RANDOMD}
cp -R ${CURPATH} ${CURPATH}_val_dbg
cd ${CURPATH}_val_dbg
...
make -j${MAKE_THREADS} | tee -a /tmp/psms_val_debug_build_${RANDOMD}
if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution | tee -a /tmp/psms_val_debug_build_${RANDOMD}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for ./scripts/make_binary_distribution!"; exit 1; fi
fi

TAR_val_dbg=`ls -1 *.tar.gz | grep -v "boost" | head -n1`
if [[ "${TAR_val_dbg}" == *".tar.gz"* ]]; then
  DIR_val_dbg=$(echo "${TAR_val_dbg}" | sed 's|.tar.gz||')
  TAR_val_dbg_new=$(echo "${PREFIX}-${TAR_val_dbg}" | sed 's|.tar.gz|-val-debug.tar.gz|')
  DIR_val_dbg_new=$(echo "${TAR_val_dbg_new}" | sed 's|.tar.gz||')
  if [ "${DIR_val_dbg}" != "" ]; then rm -Rf ../${DIR_val_dbg}; fi
  if [ "${DIR_val_dbg_new}" != "" ]; then rm -Rf ../${DIR_val_dbg_new}; fi
  if [ "${TAR_val_dbg_new}" != "" ]; then rm -Rf ../${TAR_val_dbg_new}; fi
  mv ${TAR_val_dbg} ../${TAR_val_dbg_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for moving of tarball!"; exit 1; fi
  cd ..
  tar -xf ${TAR_val_dbg_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for tar!"; exit 1; fi
  mv ${DIR_val_dbg} ${DIR_val_dbg_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for moving of tarball (2)!"; exit 1; fi
  echo $CMD > ${DIR_val_dbg_new}/BUILD_CMD_CMAKE
  echo "Done! Now run;"
  echo "mv ../${DIR_val_dbg_new} /sda"  # The script will end still in $PWD, hence we will need ../ (output only)
  #rm -Rf ${CURPATH}_val_dbg  # Best not to delete it; this way gdb debugging is better quality as source will be available!
  exit 0
else
