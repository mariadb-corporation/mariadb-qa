...
cd ..
rm -Rf ${CURPATH}_val_opt
rm -f /tmp/psms_val_opt_build_${RANDOMD}
cp -R ${CURPATH} ${CURPATH}_val_opt
cd ${CURPATH}_val_opt
...
if [ $FB -eq 0 ]; then
  ...
  echo $CMD
  $CMD | tee /tmp/psms_val_opt_build_${RANDOMD}
...
make -j${MAKE_THREADS} | tee -a /tmp/psms_val_opt_build_${RANDOMD}
if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution | tee -a /tmp/psms_val_opt_build_${RANDOMD}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for ./scripts/make_binary_distribution!"; exit 1; fi
fi

TAR_val_opt=`ls -1 *.tar.gz | grep -v "boost" | head -n1`
if [[ "${TAR_val_opt}" == *".tar.gz"* ]]; then
  DIR_val_opt=$(echo "${TAR_val_opt}" | sed 's|.tar.gz||')
  TAR_val_opt_new=$(echo "${PREFIX}-${TAR_val_opt}" | sed 's|.tar.gz|-val-opt.tar.gz|')
  DIR_val_opt_new=$(echo "${TAR_val_opt_new}" | sed 's|.tar.gz||')
  if [ "${DIR_val_opt}" != "" ]; then rm -Rf ../${DIR_val_opt}; fi
  if [ "${DIR_val_opt_new}" != "" ]; then rm -Rf ../${DIR_val_opt_new}; fi
  if [ "${TAR_val_opt_new}" != "" ]; then rm -Rf ../${TAR_val_opt_new}; fi
  mv ${TAR_val_opt} ../${TAR_val_opt_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for moving of tarball!"; exit 1; fi
  cd ..
  tar -xf ${TAR_val_opt_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for tar!"; exit 1; fi
  mv ${DIR_val_opt} ${DIR_val_opt_new}
  if [ $? -ne 0 ]; then echo "Assert: non-0 exit status detected for moving of tarball (2)!"; exit 1; fi
  echo $CMD > ${DIR_val_opt_new}/BUILD_CMD_CMAKE
  echo "Done! Now run;"
  echo "mv ../${DIR_val_opt_new} /sda"  # The script will end still in $PWD, hence we will need ../ (output only)
  #rm -Rf ${CURPATH}_val_opt  # Best not to delete it; this way gdb debugging is better quality as source will be available!
  exit 0
else
