...
buildall(){  # Build 2-by-2 in reverse order to optimize initial time-till-ready-for-use (newer builds=larger=longer)
  if [ ${BUILD_13_1} -eq 1 ]; then
    if [ ! -z "${ADD_EXTRA_AUTO_OPTIONS}" -a "${USE_EXTRA_A_OPT_13_1}" -eq 1 ]; then
      export EXTRA_AUTO_OPTIONS="${ADD_EXTRA_AUTO_OPTIONS}"
    else
      export -n EXTRA_AUTO_OPTIONS; EXTRA_AUTO_OPTIONS=
    fi
    cleanup_dirs; cd ${DIR}/13.1 && ~/mariadb-qa/build_mdpsms_opt.sh
    cleanup_dirs; cd ${DIR}/13.1 && ~/mariadb-qa/build_mdpsms_dbg.sh
  fi
  ...
  if [ ${BUILD_ES_12_3} -eq 1 ]; then
    if [ ! -z "${ADD_EXTRA_AUTO_OPTIONS}" -a "${USE_EXTRA_A_OPT_ES_12_3}" -eq 1 ]; then
      export EXTRA_AUTO_OPTIONS="${ADD_EXTRA_AUTO_OPTIONS}"
    else
      export -n EXTRA_AUTO_OPTIONS; EXTRA_AUTO_OPTIONS=
    fi
    cleanup_dirs; cd ${DIR}/12.3-es && ~/mariadb-qa/build_mdpsms_opt.sh
    cleanup_dirs; cd ${DIR}/12.3-es && ~/mariadb-qa/build_mdpsms_dbg.sh
  fi
  ...
  if [ ${BUILD_12_3} -eq 1 ]; then
    ...
    cleanup_dirs; cd ${DIR}/12.3 && ~/mariadb-qa/build_mdpsms_opt.sh
    cleanup_dirs; cd ${DIR}/12.3 && ~/mariadb-qa/build_mdpsms_dbg.sh
  fi

  if [ ${BUILD_12_2} -eq 1 ]; then
    if [ ! -z "${ADD_EXTRA_AUTO_OPTIONS}" -a "${USE_EXTRA_A_OPT_12_2}" -eq 1 ]; then
      export EXTRA_AUTO_OPTIONS="${ADD_EXTRA_AUTO_OPTIONS}"
    else
      export -n EXTRA_AUTO_OPTIONS; EXTRA_AUTO_OPTIONS=
    fi
    cleanup_dirs; cd ${DIR}/12.2 && ~/mariadb-qa/build_mdpsms_opt.sh
    cleanup_dirs; cd ${DIR}/12.2 && ~/mariadb-qa/build_mdpsms_dbg.sh
  fi
  ...
  if [ ${BUILD_ES_11_8} -eq 1 ]; then
    if [ ! -z "${ADD_EXTRA_AUTO_OPTIONS}" -a "${USE_EXTRA_A_OPT_ES_11_8}" -eq 1 ]; then
      export EXTRA_AUTO_OPTIONS="${ADD_EXTRA_AUTO_OPTIONS}"
    else
      export -n EXTRA_AUTO_OPTIONS; EXTRA_AUTO_OPTIONS=
    fi
    cleanup_dirs; cd ${DIR}/11.8-es && ~/mariadb-qa/build_mdpsms_opt.sh
    cleanup_dirs; cd ${DIR}/11.8-es && ~/mariadb-qa/build_mdpsms_dbg.sh
  fi
