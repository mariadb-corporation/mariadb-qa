...
# ub/asan's <ver>_dbg_san / <ver>_opt_san, so no scratch clash). Still do NOT run this concurrently
# with buildall_san_slow.sh: both operate on the same <ver> source trees, which this one re-clones and removes.
...
DIR=${PWD}
cleanup_dirs(){  # $1 = version dir; remove ONLY this version's MSAN build scratch (_dbg_msan/_opt_msan, distinct from ub/asan's _dbg_san/_opt_san). No generic tarball move; the MSAN_* basedirs and their tarballs stay in ${DIR}.
...
build_ver(){  # $1 = version dir under ${DIR} (e.g. 13.1, 12.3-es)
  ...
  cleanup_dirs "${ver}"
  [ ${REMOVE_SOURCE_AFTER} -eq 1 ] && rm -Rf "${DIR}/${ver}"
}
