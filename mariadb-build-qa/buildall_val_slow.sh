...
buildall(){  # Build 2-by-2 in reverse order to optimize initial time-till-ready-for-use (newer builds=larger=longer)
  ...
  if [ ${BUILD_ES_12_3} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/12.3-es && ~/mariadb-qa/build_mdpsms_opt_valgrind.sh
    cleanup_dirs; cd ${DIR}/12.3-es && ~/mariadb-qa/build_mdpsms_dbg_valgrind.sh
  fi
  ...
  if [ ${BUILD_12_1} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/12.1 && ~/mariadb-qa/build_mdpsms_opt_valgrind.sh
    cleanup_dirs; cd ${DIR}/12.1 && ~/mariadb-qa/build_mdpsms_dbg_valgrind.sh
  fi

  if [ ${BUILD_12_0} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/12.0 && ~/mariadb-qa/build_mdpsms_opt_valgrind.sh
  ...
  if [ ${BUILD_11_8} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/11.8 && ~/mariadb-qa/build_mdpsms_opt_valgrind.sh
    cleanup_dirs; cd ${DIR}/11.8 && ~/mariadb-qa/build_mdpsms_dbg_valgrind.sh
  fi

  if [ ${BUILD_11_7} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/11.7 && ~/mariadb-qa/build_mdpsms_opt_valgrind.sh
    cleanup_dirs; cd ${DIR}/11.7 && ~/mariadb-qa/build_mdpsms_dbg_valgrind.sh
  fi
