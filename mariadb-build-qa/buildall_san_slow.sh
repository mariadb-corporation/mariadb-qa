...
buildall(){  # Build 2-by-2 in reverse order to optimize initial time-till-ready-for-use (newer builds=larger=longer)
  if [ ${BUILD_12_3} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/12.3 && ~/mariadb-qa/build_mdpsms_opt_san.sh
    cleanup_dirs; cd ${DIR}/12.3 && ~/mariadb-qa/build_mdpsms_dbg_san.sh
  fi

  if [ ${BUILD_12_2} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/12.2 && ~/mariadb-qa/build_mdpsms_opt_san.sh
    cleanup_dirs; cd ${DIR}/12.2 && ~/mariadb-qa/build_mdpsms_dbg_san.sh
  fi
  ...
  if [ ${BUILD_ES_11_8} -eq 1 ]; then
    cleanup_dirs; cd ${DIR}/11.8-es && ~/mariadb-qa/build_mdpsms_opt_san.sh
    cleanup_dirs; cd ${DIR}/11.8-es && ~/mariadb-qa/build_mdpsms_dbg_san.sh
  fi
