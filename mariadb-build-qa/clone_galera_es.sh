...
clone_es_mdg_repo(){
  ...
  if [ "${2}" != "automation" ]; then  # Automation helper for cloneall_galera_es.sh
    if [[ ${1} =~ 10.[2-3] ]]; then
      rm -Rf galera_3x
      git clone --depth=1 --recurse-submodules -j8 --branch=es-mariadb-3.x https://github.com/mariadb-corporation/es-galera.git galera_3x
    else
      rm -Rf galera_4x
      git clone --depth=1 --recurse-submodules -j8 --branch=es-mariadb-4.x https://github.com/mariadb-corporation/es-galera.git galera_4x
    fi
  fi
}
