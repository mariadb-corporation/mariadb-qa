...
if [ "${1}" == "SAN" ]; then
  ...
  ls --color=never -d EMD*1[0-1].[0-9]* 2>/dev/null | grep -vE "${REGEX_EXCLUDE}" | grep -v tar | grep -vE "SAN|GAL"
  ls --color=never -d MD*1[0-1].[0-9]* 2>/dev/null | grep -vE "${REGEX_EXCLUDE}" | grep -v tar | grep -vE "SAN|GAL"
