...
DuckDB: Roman Nozdrin (`drrtuy`) is the assignee for DuckDB issues. A bug that loses or corrupts DuckDB data goes to him even when the code at fault sits in another subsystem, as with a `mariadb-backup` gap.
