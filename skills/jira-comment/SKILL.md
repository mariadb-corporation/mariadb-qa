...
- Facts only. A MENT or MDEV comment can be read by the customer and many others, so it carries what was run, what came out and what the code does, and nothing else. Nothing which reads as criticism even if true: not what a defect, a message or a wait cost anyone, not a verdict on our product, a customer, another team's work or anyone's handling of the case. We all make mistakes, including AI. Leave the impact out rather than imply it. An internal technical and facts-based judgement, if ever truly necessary, goes in a dev-only comment (`~/jira --comment KEY --dev-only`) or off the ticket, and always needs to be approved and signed-off upon, alike to all other comment posted.
