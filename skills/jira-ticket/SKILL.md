...
2. From that `bs` BUG REPORT block, take, in order: the **SAN stack** (`{noformat:title=... UBASAN ...}` ... `==...==ABORTING` `{noformat}`); the **`Setup:`** block (`Setup:` + `{noformat}` ... `{noformat}`); the **`{noformat:title=SAN Bug Detection Matrix}`** ... `{noformat}` block.
   The SAN stack block is the ONE sanctioned hand-assembled block (the agreed exception to the no-hand-rolling hard rule), because its extraction tooling is not yet fully reliable: when `bs`'s emitted SAN stack is incomplete or imperfect, build that block yourself - the `{noformat:title=...}` line from `~/myver` run in the SAN basedir, the content from the SAN report section of `log/master.err` there. Only this block; every other block stays tool-emitted verbatim.
3. Splice all three **after the regular Bug Detection Matrix**, in the order SAN stack, `Setup:`, SAN Bug Detection Matrix - the SAN matrix ends the body. Keep the blocks verbatim. The regular Bug Detection Matrix stays where `~/b` put it, directly under the crash stack. Full body order when a SAN issue fired: testcase > assert/crash stack > regular Bug Detection Matrix > SAN stack (from the error log) > `Setup:` > SAN Bug Detection Matrix.
