...
- **Do the whole job before you present, on every disposition.** ONE approval gate, not three. Never present a bare decision menu and wait, then draft the comment after the answer, then plan the version edit after that - each round trip costs the user real time and they have said so. By the time you present, the reduction is done, the MTR is verified, the matrix is swept, the comment text is drafted verbatim, and the version-edit command is written out. The user then answers once and everything executes. Stop early ONLY at a real fork you cannot resolve, and say in one line what you would do by default.
...
| `~/stack` | the Jira-ready stack block for the core in the current dir; the ONLY way to produce a stack for a report or comment |
...
| `~/eb` / `~/kb` / `~/kba` / `ca` | register testcase (vi, then `git add` on exit) / known UniqueID / SAN UniqueID / clean known trials - `ca` is automatic while a `my_cleanup_script` screen runs |
...
1. **Find the basedir.** The trial dir is `<workdir>/<trialnum>`; from the workdir run `~/i <trialnum> | grep -E 'BASEDIR|REPLICATION'` (do NOT grep `reducer<N>.sh` - its `BASEDIR=` is a `<set_below...>` placeholder; and `~/i` must run from the workdir, not inside the trial dir). The reproducing basedir is often a `UBASAN_`/`MSAN_` build - for the regular `~/b` report strip that prefix to its non-SAN sibling of the same version (`UBASAN_MD050626-...-13.1.0-...-dbg` -> `MD050626-...-13.1.0-...-dbg`); keep the SAN basedir for step 5 (`~/bs`). If the wanted basedir is gone, pick the closest version from `cd /test && bash gendirs.sh`. Prefer the **dbg** basedir of the reproducing version (asserts/SIGABRT and clean frames come from dbg), EXCEPT where step 1a shows the target UniqueID only appears in `opt` - then reduce in `opt`. Replication trials show `MASTER_EXTRA`/`SLAVE_EXTRA`/`REPL_EXTRA` lines in `~/i <trialnum>` - treat those as REPLICATION=1.

1a. **The target UniqueID is the one `pr` shows, and BOTH the `opt` and the `dbg` UniqueID must be captured** (here, and dedup'd separately in step 3).

   `pr`, run in the workdir, lists the UniqueID the run saved. That exact string is the reduction target: if reduction ends on a different UniqueID, the result is a different bug and belongs to a different ticket. Reduce against the basedir flavour the run used, so the UID stays comparable.

   Run the finished testcase in the `opt` AND the `dbg` basedir of the same version and record `~/t` from each. **The two builds routinely report DIFFERENT UniqueIDs for one testcase**, because a `DBUG_ASSERT` present only in `dbg` fires earlier on the same code path and masks a release-only `ut_a`/SIGSEGV further down, while `opt` does not contain that assertion and runs on to the later abort. One build can therefore read "NEW" while the other is an exact `## MDEV` hit, for what is one and the same bug.

   Both readings are load-bearing. If EITHER UniqueID is a live `## MDEV` hit, the finding belongs to that MDEV: the other build's UniqueID is the same defect seen in a different build mode, not a second bug, so the disposition is a comment on that MDEV rather than a new ticket. What makes such a comment worth posting is the delta - most often that the variant crashes an OPTIMIZED build where the ticket's own testcases do not. Establish that by running the ticket's registered testcases from `~/mariadb-qa/BUGS/MDEV-<key>.sql` in the `opt` basedir. MDEV-24808 is a clear illustration: a long UNIQUE index on a partitioned table asserts in `handler::ha_index_read_map` in `dbg`, and an `ON DUPLICATE KEY UPDATE` variant of it reaches an InnoDB `ut_a` abort in `opt`.

   Carry both UniqueIDs, labelled by build, into the step 8 overview and into the report body.
...
**Run this whole step ONCE PER BUILD FLAVOUR the testcase crashes** - `~/tt` in the `opt` basedir and `~/tt` in the `dbg` basedir (plus the SAN basedir when a SAN UniqueID fired). Per step 1a the two flavours often carry different UniqueIDs for one testcase, and a single-flavour dedup is how an existing ticket gets missed and a duplicate filed. A live `## MDEV` hit on ANY flavour decides the disposition for the whole finding.
...
- **Likely duplicate, or fixed-but-reproduces** -> STOP, do NOT file a new ticket, and present the user a short menu. For a duplicate the DEFAULT is to register (not delete):
  ...
  | Comment to the ticket (post for a stack variant or a substantially different testcase) | Post a note when EITHER our testcase SUBSTANTIALLY differs from the dup's (different SQL structure, even if the UID is identical) OR ours is a **stack variant** of the dup (a different UID / frames / call path into the SAME bug) - a variant stack tells the dev the bug is reachable another way, so a variant USUALLY warrants a short note even when the SQL is similar. Intro `Ran into this one also. CLI testcase:` (label `CLI testcase` unless you have verified it in MTR) + the testcase (`{code:sql}`) + `Leads to:` + `~/stack` output, verbatim - it already supplies the `{noformat:title=<version banner>}` header, so never write a title yourself or paste a bare UniqueID into one. If the STACK/UID differs from the ticket's, frame the intro as `Additional testcase with slight stack variation:` and give the intro no mechanism explanation - the stack and the testcase carry it. **A comment is leaner than a report - it carries only what the ticket does not already have:**

  - **dbg stack only.** Add the opt stack ONLY when it is new compared with what the ticket already carries. Do not post both as a matter of course.
  - **Never repeat a stack the ticket already has.** When the stack you would post is already there, drop the block and write one line instead: `Same stack as the one above`, or `Same stack as one of the ones above` when the ticket carries several. When both your dbg and opt stacks are already there, that single line replaces both blocks - post the testcase, the line, and the matrix.
  - **Never include the `Setup:` block.** It is report-only; in a comment it is noise.
  - **A *SAN block goes in only when the ticket has no SAN data for this path.** Then it is `~/stack`'s sanitizer block, on its own, without `Setup:`.
  - **Bug Detection Matrix and SAN Bug Detection Matrix: include where relevant** - always when the comment says anything about which versions are affected, and alongside a SAN block for the SAN one. ALWAYS end the comment with the line `Please also test any fixes with this testcase.` (so the dev validates the fix against this variant too). That line belongs ONLY to a comment on a PRE-EXISTING bug - one somebody else already filed, where our testcase is an extra variant the dev has to cover too. Never put it in a new ticket's description, and never in a comment on a ticket filed in this session. "Substantially differs" is judged against ALL existing testcases in the issue - the description AND every comment (some may be the user's own); read them first, and post only when this testcase adds a genuinely new path/index/version (e.g. DESC vs ASC, skip_nulls vs next_min, a newer regression range). Present the LITERAL text for approval, then `~/jira --comment <KEY> --description-file <file> -y`; NEVER auto-post (hard rule). Minor diffs only -> skip the comment (the eb-append already captured it). |
  | Extend the dup's versions | When our repro confirms the bug live on a NEWER mainline GA than the issue lists (e.g. it lists up to `12.1(EOL)` but crashes on `13.0`/`13.1` builds), ADD those versions to the existing issue rather than commenting the same testcase. **All THREE version fields move together - Affects, Affects ES and Fix Version.** Extending Affects while leaving Fix Version behind is an incomplete edit and the user has had to finish it by hand. Derive Fix the same way as for a new ticket: every affected CS branch except the single newest, which gets it by up-merge. So a run confirming 12.3, 13.0 and 13.1 adds all three to Affects and adds 12.3 and 13.0 to Fix. `~/jira --edit <KEY> --affects-version 12.3 --affects-version 13.0 --affects-version 13.1 --es-version 12.3 --fix-version 12.3 --fix-version 13.0` (additive PUT, never replaces; **mainline X.Y names only - `13.0`, NOT `13.0.1`**; Fix excludes the newest branch, which gets the fix by up-merge). `-y` to skip the 3x prompt for this directly-instructed edit. Validate each name exists via `/rest/api/2/project/MDEV/versions` first. Prefer this over a comment when the testcase/mechanism is the SAME and only the affected range differs. |
  | File as a new/distinct bug | Genuinely distinct, or the match is CLOSED and still reproduces (incomplete fix / missed variant / regression) -> proceed to step 4, and link the closed key as `Relates`. A closed ticket is never reopened, so this is the ONLY route for a post-fix reproducer. |
  ...
  **Bring the finished work with the menu, not after it.** A duplicate is not a reason to present less. Before showing the menu, complete every artifact the recommended option needs, so a single answer is enough to execute:

  - the hand-reduced testcase, re-verified;
  - the MTR testcase, verified as a reverse gate - failing on an affected build AND passing where the behaviour is correct (the reference vendor's build serves for that when no fixed build exists);
  - the Bug Detection Matrix, swept over the `gendirs.sh` set;
  - the comment text drafted IN FULL and shown verbatim, per the comment rules above and the voice profile;
  - the exact `~/jira --edit <KEY> --affects-version ... --es-version ...` command for the version extension;
  - the `BUGS/<KEY>.sql` content, ready to write.

  Where a drafted line depends on which option is chosen (a comment saying "I have added these to the ticket" when the version edit may not happen), say so in one line beside the draft and give the alternative wording. Do not use it as a reason to postpone the draft.
...
- **Framework-logged** (loggers who use `b`/`br`/`bs`; the tell is a bug-report-style body: `{code:sql}` testcase + version build-tag `{noformat}` blocks + a Bug Detection Version Matrix): their UniqueID is already registered in `kb`/`kba`, so `tt`'s String Scan will recognise it.
...
**Prose paragraphs must be a single unwrapped line.** `~/b` hard-wraps the explanatory prose; un-wrap each prose paragraph to one line (only `{code}`/`{noformat}` blocks keep their line structure). **Never clobber the user's edits on a filed ticket:** there is no `~/jira` description-edit, so a description change needs a raw `PUT .../rest/api/2/issue/KEY` with `{fields:{description:...}}` (jq `-Rs` to encode the body) - before any full re-PUT, re-fetch the live description and carry forward any manual edits the user made, then sync the local `log_jira_ticket.body`/`.txt`.

**Never replace a tool-emitted block with your own prose, and never drop one.** The body keeps the `~/b` shape exactly, in this order: testcase, `Leads to:`, crash stack, Bug Detection Matrix, then (when a SAN issue fired) SAN stack, `Setup:`, and the SAN Bug Detection Matrix LAST. The Bug Detection Matrix is ALWAYS present and always follows the assert/crash stack - its rows ARE how CS and ES version coverage is stated, so a sentence like "also reproducing on ES 10.6, 11.4, 11.8" is wrong and must not appear in place of (or alongside) the matrix. If a matrix row looks wrong - a `No bug found` cell that a direct re-run contradicts - the fix is to RE-RUN `~/b` and regenerate the matrix, never to hand-edit a row, omit the matrix, or paraphrase it. The SAN matrix stands alone ONLY for a SAN-only finding - one that does not reproduce in regular builds at all. Whenever a regular build reproduces the issue, the regular matrix covers that assert/crash and the SAN matrix follows it; the two are not alternatives.
...
**Pre-file gate - reconcile the matrix against commit / up-merge status (CRITICAL).** The matrix is a snapshot of specific build COMMITS. A build's version STRING or date does NOT prove the fix is present: a build labelled `12.3.2` but cut before the `12.3.2` fix commit still crashes, and a clean cell can mean "already fixed in that build", not "unaffected". MariaDB lands a fix in the earliest affected GA branch and up-merges OLDEST->NEWEST in stages, giving two traps:
...
- Run `~/stack` in each build that fires and use its output verbatim. It emits the assert block plus ONE stack, each with its own `{noformat:title=<version banner>}` - the *SAN / Valgrind block on a sanitizer build, otherwise the gdb backtrace. Never read `log/master.err` yourself to build either.
...
### Step 4b - When `~/b` cannot run

First check that it really cannot. Plain `b` looks for crashes and asserts in the error logs as well as for core files. `bs`/`bm`/`bt`/`bv` take a search string as their argument, so a sanitizer or Valgrind text works. `bug_report.sh` itself honours `export TEXT='<string>'` for any other error-log text.

Only when none of those fits - a wrong result that leaves nothing in the error log, for instance - build a `b`-like report the same way the framework builds one: read `~/mariadb-qa/bug_report.sh` and the matrix chain it uses (`test_all` runs `/test/findbug_new`, which drives `gendirs.sh` plus each basedir's `./line`), and follow that output. Do not define a report or matrix layout here.
...
2. From that `bs` BUG REPORT block, take, in order: the **SAN stack** (`{noformat:title=... UBASAN ...}` ... `==...==ABORTING` `{noformat}`); the **`Setup:`** block (`Setup:` + `{noformat}` ... `{noformat}`); the **`{noformat:title=SAN Bug Detection Matrix}`** ... `{noformat}` block.
   If `bs`'s emitted SAN stack looks incomplete, do NOT rebuild it by hand. Run `~/stack` in that SAN basedir: it emits the sanitizer block on its own, from the first sanitizer issue line through its closing `SUMMARY:` / `==ABORTING` line, already wrapped in the `{noformat:title=<version banner>}` header. Use that output verbatim. `~/stack` covers UBSAN, ASAN, MSAN, TSAN and Valgrind, picks the right start and end markers per sanitizer, and needs no core, so it also serves a UBSAN runtime error that did not abort.
   **Take the SAN stack and NOT the SAN build's core backtrace.** The body carries one stack per build flavour: the non-SAN build contributes its crash stack, the SAN build contributes its sanitizer block. A SAN build's gdb backtrace never goes in - it usually shows only the abort path. `~/stack` already enforces this, so a `~/stack` run in a SAN basedir cannot give you the wrong one.
3. Splice all three **after the regular Bug Detection Matrix**, in the order SAN stack, `Setup:`, SAN Bug Detection Matrix - the SAN matrix ends the body. Keep the blocks verbatim. The regular Bug Detection Matrix stays where `~/b` put it, directly under the crash stack. Full body order when a SAN issue fired: testcase > assert/crash stack > regular Bug Detection Matrix > SAN stack > `Setup:` > SAN Bug Detection Matrix.
...
- **Assignee** - derive the default by the subsystem owning the bug's ROOT cause (the feature fundamentally involved, not the surface symptom). A virtual-column bug goes to the Virtual columns owner even when it surfaces via the optimizer's filesort. A sibling's assignee (from dedup) is only a weak hint, and only if it is TRULY the same bug - do NOT override the map from a loosely-related sibling. Take the Jira username from the PRE-RESOLVED list at the top of `~/mariadb-qa/skills/_shared/default_assignees.md` - do not re-derive one that is already there. For a name the file does not list, resolve via `https://jira.mariadb.org/rest/api/2/user/search?username=<prefix>` (the param is `username`, NOT `query`) and take the **`name`** field, NOT `key` - see the gotcha below. Set with `--assignee <name>`, and surface it (Display Name + username) in the overview for signoff. Consult the subsystem-to-assignee map in `~/mariadb-qa/skills/_shared/default_assignees.md` to pre-fill the suggestion.

**Username fields (gotcha, this has cost a failed filing).** `user/search` returns BOTH a `key` and a `name`, and they are NOT always the same string. Sergei Petrunia is `key=psergey` but `name=psergei`. **`--assignee` takes the `name`.** Passing the `key` fails the create with `User '<key>' does not exist.` So always read `.name`, never `.key`, for `--assignee`:

```bash
curl -sS -H "Authorization: Bearer $PAT" 'https://jira.mariadb.org/rest/api/2/user/search?username=<prefix>' \
  | jq -r '.[] | "name=\(.name)  key=\(.key)  display=\(.displayName)"'
```

Better still, do not resolve it at all: `~/mariadb-qa/skills/_shared/default_assignees.md` carries the usernames PRE-RESOLVED for every subsystem owner. Read the name out of that file and use it as-is. Only fall back to `user/search` for somebody the file does not list, and then take `.name`.

The `key` is still the right field for JQL (`reporter = <key>`, `assignee = <key>`), for `[~handle]` mentions, and for matching comment authorship. The current user's own identity comes from their PAT: `~/jira --whoami` prints it, and `curl -sS -H "Authorization: Bearer $PAT" https://jira.mariadb.org/rest/api/2/myself | jq -r .key` gives the exact lowercase key. But a comment's `author.name` can be CAPITALISED and `author.displayName` is the full display name. When matching comment authorship programmatically (e.g. "find comments by the current user"), compare `author.key` (lowercase) or case-insensitively - NEVER `author.name`, which can silently match nothing.
- **Issue type** - `Bug`.

## Step 8 - Assemble the overview and get approval

ALWAYS produce and present the full overview, **irrespective of disposition** - file, comment-on-existing, duplicate (open or closed), refuted, or any no-action outcome all get one. Never skip or shorten it because the verdict is "duplicate, do not file"; a duplicate needs as clear a paper trail (crash/UID, assert+line, trigger, reduced testcase, dedup verdict + evidence chain, matrix, explicit disposition) as a filing, and it is persisted in the dir even when nothing is filed. **Reproduce the overview IN the chat response every time - fields, dedup evidence, prose, and the CLI and MTR testcase blocks in full, inline.** Tool-emitted blocks are the one part NOT pasted into chat: at their body positions write `[assert]`, `[stack]`, `[bug_matrix]`, `[san_stack]`, `[san_setup]`, `[san_matrix]` placeholders instead - the real blocks live verbatim in `log_jira_ticket.body` (or the comment file), which is the paste-to-Jira source, and retyping them into chat is where hand-rolling drift creeps in. Square brackets, not angle brackets: `<stack>` gets HTML-escaped by the chat renderer and shows as `&lt;stack&gt;`. Do not reference "presented earlier", do not just persist it to a file, and never drop the testcase or point at the file instead of showing it (the recurring miss).
...
grep -n '\\-' "$F"                          # any \- : allowed ONLY inside a {{..}} span, nowhere else
grep -n '||' "$F"                            # Jira pipe table: never allowed, use {noformat:title=...}
grep -nP '\x{2014}' "$F"                     # em-dash: never allowed
...
Always pass `--assignee <username>` matching the `Assignee` shown in the approved `log_jira_ticket.txt` overview - the sign-off INCLUDES the assignee, so the filing must match it (do NOT file unassigned when an assignee was shown and approved). The username comes from the pre-resolved list in `~/mariadb-qa/skills/_shared/default_assignees.md`; if you resolved it yourself, confirm you took `user/search`'s **`name`** field and not its `key` - they differ for some users and the `key` fails the create.
...
**Order matters - registering DELETES the trial dir.** While a `my_cleanup_script` screen runs, `ca` sweeps on a loop, so the moment a UniqueID lands in `kb`/`kba` that screen deletes every matching trial dir, within a minute or two. It takes `log_jira_ticket.txt`, `log_jira_ticket.body`, the reduced `_out` files and the core with it. So BEFORE the `kb`/`kba` step, copy anything you still need out of the trial dir - the two `log_jira_ticket.*` files at minimum, plus the `~/b`/`~/bs` logs if they are not already elsewhere. Keep the copies in your scratchpad; do not invent a new directory under `~/mariadb-qa` or `/data` for them. Never re-read a trial dir after registering, and never point the user at a path inside one.

1. **`eb`** - write the CLI testcase (incl. any `# mysqld options required for replay:` header) to `~/mariadb-qa/BUGS/MDEV-xxxxx.sql`, one unbroken line per statement. `eb` opens that file in vi and runs `git add` on exit, so the staging is `eb`'s own behaviour, not an extra step. vi cannot run unattended, so write the file directly and run the same stage yourself: `cd ~/mariadb-qa/BUGS && git add MDEV-xxxxx.sql` (`git add` is allowed; only `git commit`/`push`/`rm` are denied). Do not write the file and leave it untracked.
...
4. **`ca`** - clean all workdirs of now-known trials (after kb/kba). **Check for the cleanup script FIRST:**

   ```bash
   screen -ls | grep -i my_cleanup_script     # e.g. 3267216.my_cleanup_script  (Detached)
   ```

   A running `my_cleanup_script` screen already sweeps the workdirs on a loop, so it picks up the entries you just added to `kb`/`kba` by itself. When that screen is there, `ca` is automatic - do NOT start a second sweep, and report it as covered by the cleanup script. Only when no such screen is running, start one yourself:
...
**Gate:** `eb` file written AND `git add`ed, `kb` and/or `kba` updated (with the `## MDEV` marker column-aligned - see below), and the trials cleaned - either by the running `my_cleanup_script` screen or by a sweep you started. Surface the staged `BUGS/MDEV-xxxxx.sql` and the modified `known_bugs.strings`/`.SAN` for the user to commit - never `git commit`/`push` yourself.

## End-of-turn report (tickboxes)

Status icons per `~/mariadb-qa/skills/_shared/tickbox_icons.md` (full set: 🟢 🟡 🟠 🔴 🔵 ✅ ❌ ⚠️ ⚖️ ⏳) - pick the icon matching each step's real outcome. The list below is the all-done path.
...
⏳ submit pending approval   (or: ✅ filed MDEV-xxxxx <url>)
   then: ✅ related links applied, eb + kb/kba registered, trials cleaned (my_cleanup_script, or a sweep started)
```

## Related

- `~/mariadb-qa/log_jira_ticket.sh` - the filing/commenting script (PAT auth, 3x confirm).
- Jira wiki markup: `~/mariadb-qa/skills/_shared/jira_markup.md`. MTR craft: the `mtr_testcase` skill.
