...
3. Splice all three **after the regular Bug Detection Matrix**, in the order SAN stack, `Setup:`, SAN Bug Detection Matrix - the SAN matrix ends the body. Keep the blocks verbatim. The regular Bug Detection Matrix stays where `~/b` put it, directly under the crash stack.
