...
cd ..
rm -Rf ${CURPATH}_opt
rm -f /tmp/mdpsms_opt_build_${RANDOMD}
...
if [ $FB -eq 0 ]; then
  ...
  # Search key: Mac
  CMD="cmake . $CLANG $AFL $SSL -DBUILD_CONFIG=mysql_release ${EXTRA_AUTO_OPTIONS} ${XPAND} -DWITH_UNIT_TESTS=0 -DWITH_TOKUDB=0 -DWITH_JEMALLOC=no -DFEATURE_SET=community -DDEBUG_EXTNAME=OFF -DWITH_EMBEDDED_SERVER=${WITH_EMBEDDED_SERVER} -DWITH_DEBUG_SYNC=ON -DENABLE_DOWNLOADS=1 ${BOOST} -DENABLED_LOCAL_INFILE=${WITH_LOCAL_INFILE} -DENABLE_DTRACE=0 -DWITH_{SAFEMALLOC,NUMA}=OFF -DWITH_UNIT_TESTS=OFF -DCONC_WITH_{UNITTEST,SSL}=OFF -DPLUGIN_PERFSCHEMA=${PERFSCHEMA} ${DBUG} ${ZLIB} -DWITH_ROCKSDB=${WITH_ROCKSDB} -DWITH_PAM=ON -DWITH_MARIABACKUP=1 -DFORCE_INSOURCE_BUILD=1 -DWITHOUT_GROUP_REPLICATION=1 ${FLAGS}"  # -DWITHOUT_PLUGIN_OQGRAPH=1 -DWITHOUT_OQGRAPH=1
...
make -j${MAKE_THREADS} 2>&1 | tee -a /tmp/psms_opt_build_${RANDOMD}
if [ "${PIPESTATUS[0]}" -ne 0 ]; then echo "Assert: non-0 exit status detected for make!"; exit 1; fi
...
if [ ! -r ./scripts/make_binary_distribution ]; then  # Note: ./scripts/binary_distribution is created on-the-fly during the make compile
  ...
  ./scripts/make_binary_distribution 2>&1 | tee -a /tmp/psms_opt_build_${RANDOMD}
