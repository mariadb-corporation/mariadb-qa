...
while IFS=$'\n' read LINE; do
  ...
  if [ ${FLAG_UBSAN_PRESENT} -eq 1 ]; then
    if [[ "${LINE}" == *"runtime error:"* ]]; then  # UBSAN Issue detected, and commencing
      ...
      UBSAN_FRAME1=; UBSAN_FRAME2=; UBSAN_FRAME3=; UBSAN_FRAME4=
      UBSAN_ERROR="$(echo "${LINE}" | sed 's|.*runtime error:|runtime error:|;s|load of value [0-9]\+|load of value X|g;s|applying non-zero offset [0-9]\+|applying non-zero offset X|g;s|shift exponent [0-9]\+|shift exponent X|g;s|index [0-9]\+ out of bounds|index X out of bounds|g')"
